...
export class CalendarService {
  ...
  createCalendarDay(time: number, opt: CalendarModalOptions, month?: number): CalendarDay {
    ...
    // console.log('RangeBeg ' + _rangeBeg + 'RangeEnd ' + _rangeEnd);
    if (_rangeBeg > 0 && _rangeEnd > 0) {
      if (!opt.canBackwardsSelected) {
        isBetween = !_time.isBetween(_rangeBeg, _rangeEnd, 'days', '[]');
        // console.log('--111--' + isBetween);
      } else {
        isBetween = moment(_time).isBefore(_rangeBeg) ? false : isBetween;
        // console.log('--2222--' + isBetween);
      }
    } else if (_rangeBeg > 0 && _rangeEnd === 0) {
      if (!opt.canBackwardsSelected) {
        let _addTime = _time.add(1, 'day');
        isBetween = !_addTime.isAfter(_rangeBeg);
        // console.log('--3333--' + isBetween);
      } else {
        isBetween = false;
        // console.log('--4444--' + isBetween);
      }
    }
    else {
      try {
        var val = moment(moment().subtract(18, 'years').valueOf()).diff(time, 'days');
        if (val <= 0) {
          if (val == 0 && moment(time).format('DD') == moment().format('DD')) {
            isBetween = false;
          }
          else {
            isBetween = true;
    ...
    let _disable = false;
    // console.log('Disable ' + moment(opt.from).format('DD MM YYYY') + 'ddd ' + moment(opt.to).format('DD MM YYYY'));
    if (dayConfig && isBoolean(dayConfig.disable)) {
      _disable = dayConfig.disable;
    } else {
      _disable = disableWee || isBetween;
    }
    // console.log('Disable ' + _disable + 'disableWee ' + disableWee + 'isBetween ' + isBetween);
    ...
    let subTitle = moments(time).iDate().toString();
    // console.log("subtitle", subTitle + 'Time ' + moment(time).format('DD MM YYYY'));
    if (dayConfig && dayConfig.subTitle) {
      subTitle = dayConfig.subTitle;
    } else if (opt.defaultSubtitle) {
      if (opt.defaultSubtitle == 'no') {
        subTitle = '';
      }
    }
